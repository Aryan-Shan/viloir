...
async function initializePeerJS() {
  try {
    ...
    // Initialize PeerJS with STUN and TURN server integration
...
function initializeInCallField() {
  ...
  set(userStatusRef, {
    ...
    .then(() => {
      console.log(
        "inCall field initialized to false for user:",
        currentUser.uid
      );
    })
    .catch((err) => {
...
function initiateOutgoingCall(targetUserId) {
  ...
  getUserEmail(targetUserId, (targetUserEmail) => {
    ...
    call.on("stream", (remoteStream) => {
      ...
      hideLoading();
      connectedUserDisplay.textContent = `Connected with: ${targetUserEmail}`;
